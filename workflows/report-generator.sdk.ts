...
// Webhook POST {campaign_lead_id} -> Load Report Data (business + current assessment +
...
const note = sticky('## Report Generator\n\nPOST {campaign_lead_id} -> load evidence -> Gemini pitch (tailored to best_angle) -> build self-contained HTML -> SigV4 upload to S3 (public-read, unguessable key = secret link) -> record_lead_report (DB copy + URL) -> returns URL. Bucket n8n-leadgen-reports (us-east-1). Design "clinical luxury"; agency identity ("Northlight Studio") is a placeholder to swap for the real brand.', [load], { color: 4 });
