...
**Soft-Archiving System (2026-07-21)** — Implemented non-destructive soft-archiving (`archived_at timestamptz NULL`) across evidence items, campaign leads, work items, and campaigns. RPC functions `leadgen.archive_evidence`, `leadgen.archive_lead`, `leadgen.archive_campaign`, and `leadgen.unarchive_evidence` allow archiving data without hard-deleting rows. Scorer and Report Generator filter `WHERE archived_at IS NULL`, causing fresh campaigns to ignore archived evidence and re-collect live evidence from providers. An **Archive** button on lead cards in Ops Console allows soft-archiving directly from the UI.

**Sales-effectiveness v3 & Bing Ads Verification (2026-07-21)** — Ads/Social scoring now includes full Bing/Microsoft Ads verification: Website Auditor detects Bing UET pixel tags (`bat.bing.com`, `uetq`, `bat.js`), and Ad Verification queries SerpApi Bing Search Ads (`engine: 'bing'`) to confirm active campaigns and populate `summary.bing`. Phone Probe Service logs call transcripts, Twilio AMD status, and Gemini audio prompts into `phone_probe_log` evidence. The Ops Console includes a `{ } Debug Logs` visual debugger modal with tabs for Bing & multi-channel ads, phone probe logs, LLM feeds/prompts, and raw JSON evidence ledgers.
Ads/Social scoring uses evidence the workers actually produce: verified `ad_active=false` and weak homepage-linked social footprint (`social_platform_count<2`). Late `ads_evidence` and `competitors_evidence` route back to assessment, and deep re-analysis includes both services. Competitor candidates are merged from campaign peers + Google Places, deduplicated, and ranked by a relevance composite (market strength, proximity, comparable review scale, and same-campaign/category confidence), rather than review strength alone. Reports load only provider facts or latest-confirmed evidence, exclude the target from peer stats, include median/quartiles/percentile, lead with the best-angle action plan, explicitly position managed social plus Google/Yelp/Nextdoor campaigns, validate grounded JSON, retry once, and fall back deterministically. Published report records retain the assessment/evidence/model/validation snapshot. “Not detected” is never presented as proof of absence; Nextdoor remains pixel/detection-only.
