...
// TwiML Ask endpoint handler: Serves interactive question to the callee upon call connect.
// Initial <Pause length="3"/> (2.5-3s buffer) allows callee to pick up handset & complete greeting.
// <Say voice="Polly.Generative-Joanna"> provides human-grade natural speech.
const twimlAskHook = trigger({ type: 'n8n-nodes-base.webhook', version: 2.1, config: { name: 'TwiML Ask', position: [200, 600], parameters: { httpMethod: 'POST', path: 'leadgen-phone-twiml-ask', responseMode: 'responseNode' } }, output: [{}] });

const twimlAskResp = node({ type: 'n8n-nodes-base.respondToWebhook', version: 1.1, config: { name: 'TwiML Ask Response', position: [440, 600], parameters: { respondWith: 'text', responseBody: '<?xml version="1.0" encoding="UTF-8"?><Response><Pause length="3"/><Say voice="Polly.Generative-Joanna">Hi there! I am doing a quick check—am I speaking with an automated assistant, or a real person?</Say><Record maxLength="6" timeout="3" playBeep="false" trimSilence="true"/><Hangup/></Response>', options: { responseHeaders: { entries: [ { name: 'Content-Type', value: 'application/xml' } ] } } } } });
...
const buildEv = node({ type: 'n8n-nodes-base.code', version: 2, config: { name: 'Build Probe Evidence', position: [1800, 220], parameters: { mode: 'runOnceForEachItem', language: 'javaScript', jsCode: "const poll=$('Poll Call').item.json; const cls=$json; const claim=$('Claim Probe Work').item.json; const tgt=$('Get Target').item.json; const cid=claim.campaign_id, bid=tgt.business_id; const ab=(poll.answered_by||'').toString(); const status=(poll.status||'').toString(); const pat=(cls.phone_assistant_type||'unclear').toString(); const isAi=pat==='ai_assistant'; const unanswered=isAi||['voicemail','no_answer'].includes(pat); function mk(fk,val,vt){ return {feature_key:fk,value:val,value_type:vt,product_tag:'voice_ai',source_provider:'phone_probe',idempotency_key:cid+':'+bid+':'+fk}; } const logVal={ phone_e164:tgt.phone_e164||null, twilio_call_sid:poll.sid||null, answered_by:ab||'unknown', phone_assistant_type:pat, words_spoken_transcript:cls.words_spoken||cls.transcript||cls.greeting_text||null, llm_audio_prompt:'Decide based ONLY on WHAT THE BUSINESS LINE SAYS (the words spoken), NOT on how natural or human the voice sounds. If the line states/implies it is an AI, automated, or virtual assistant -> ai_assistant', llm_raw_response:cls }; const ev=[ mk('phone_unanswered', unanswered, 'boolean'), mk('phone_probe_answered_by', (ab||'unknown'), 'enum'), mk('phone_assistant_type', pat, 'enum'), mk('phone_probe_log', logVal, 'object') ]; return { json:{ payload:{ cause_type:'phone_evidence', evidence:ev, run:{ workflow_version:'phone-probe-svc-v3.2', answered_by:ab, status:status, phone_assistant_type:pat, log:logVal } } } };" } }, output: [{ payload: {} }] });

// Classify Greeting — deployed node: fetches Twilio recording MP3 -> Gemini Flash inline audio,
// transcribes verbatim speech -> words_spoken / transcript AND classifies phone_assistant_type.
const classify = node({ type: 'n8n-nodes-base.code', version: 2, config: { name: 'Classify Greeting', position: [1690, 220], parameters: { mode: 'runOnceForEachItem', language: 'javaScript', jsCode: "// REDACTED (runtime): fetch <<TWILIO recording>> mp3 (basic auth) -> Gemini flash inline audio,\n// verbatim transcript + content-based prompt -> phone_assistant_type & words_spoken.\nreturn { json:{ phone_assistant_type:'human', words_spoken: 'Hello, how can I help you?', transcript: 'Hello, how can I help you?' } };" } }, output: [{ phone_assistant_type: 'human', words_spoken: 'Hello', transcript: 'Hello' }] });
...
const note = sticky('## Phone Probe Service V3.2 (warm-gated phone_probe, INTERACTIVE)\n\nClaim phone_probe -> Get phone -> Twilio call + AMD, TwiML /leadgen-phone-twiml-ask (2.5s pause + Polly.Generative-Joanna voice) asks "AI or a real person?" + Records -> Wait 45s -> poll -> Classify Greeting (Gemini audio, decide by WORDS not voice tone) -> phone_assistant_type + phone_unanswered (voice_ai +15) + phone_probe_answered_by -> complete (cause phone_evidence) -> re-score. Report renders "How your phone is answered". Warm/hot gated. Ships DISABLED (real recorded calls). Twilio via the n8n credential.', [claim], { color: 6 });
...
export default workflow('leadgen-phone-probe-service', 'Leadgen — Phone Probe Service')
  ...
  .add(twimlAskHook).to(twimlAskResp)
  .add(note);
