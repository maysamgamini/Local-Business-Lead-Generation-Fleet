...
- [~] T041 [US1] `workflows/sweeper.json` — scheduled engine calls (T019/T021 reconciliation), deadline enforcement, fenced finalization, quality_state, Sheets snapshot export. **DEPLOYED (f5xBdfjMchJgJOzq)**: maintenance + disabled-service skip (Option A) + finalization pass (begin/complete_campaign_finalization, deadline resolution, quality_state) wired & published. Sheets snapshot deferred (no Google cred; US4-adjacent). NOT yet proven end-to-end — blocked by the T040 assessment gaps below.
  - **Option A (data-driven service enablement)** shipped to unblock US1 finalization: `db/migrations/110_service_enabled.sql` (service_config.enabled; reviews/phone/enrichment/assets=false) + `commit_discovery_results` gates the lead work-item graph by enabled + Sweeper skips existing nonterminal items for disabled services.
  - **T040 follow-up bugs discovered (block finalization; assessment/Scorer layer):** (a) **no-website leads never get an initial assessment** — assessment only unblocks on an analyzer completion, but a no-domain lead has no analyzer to complete; product-critical (website_present=false is the top redesign signal). (b) `requeue_stale_assessments` excludes `state='dead'`, so a dead assessment never revives when lead_revision advances past its watermark. (c) work_item.completed_version vs lead_assessments.evidence_watermark can diverge.
