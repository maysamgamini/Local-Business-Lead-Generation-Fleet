...
- [~] T076 [Phone V2] **Phone Probe (on-demand)** `workflows/phone-probe.sdk.ts` (`8JLoJMdcFY8ylhzI`) — Twilio AMD probe-caller: POST {to} → originate call + Answering-Machine-Detection → Wait 45s → poll answered_by → classify (human / machine-voicemail / fax / no-answer) as the AI-receptionist signal; TwiML pause+hangup (no speech/recording). Twilio upgraded from trial (Full). v1 returns the classification; **NOT yet writing scoring evidence** (evidence_items.service_run_id NOT NULL → needs the work-queue). v2 = warm-gated `phone_probe` fleet service writing phone_evidence on Phone Presence V1's contract.
