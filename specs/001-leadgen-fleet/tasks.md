...
- [~] T052 [US3] **DESCOPED (2026-07-19, user decision)** — Google Sheets is a human-convenience input surface only; the authenticated **API intake (T053)** covers programmatic/standing campaigns and **Postgres** is the source of truth. Standing/scheduled runs, if needed, can be driven by a cron hitting the API. (Was: `workflows/intake-schedule.json` ICP-sheet reader + `claim_scheduled_profile_slot()`.)
...
- [~] T056 [US4] **DESCOPED (2026-07-19, user decision)** — Slack is a human push-notification surface; the milestone events already persist in `outbox_events` (Postgres). If live alerts are wanted later, wire email or a lightweight channel off the existing event-relay — no new vendor needed. (Was: Slack milestone destinations in event-relay.)
...
- [~] T058 [US4] **DESCOPED (2026-07-19, user decision)** — Airtable is a no-SQL browse UI over data Postgres already holds; replaceable by a lightweight dashboard/Metabase or the HTML reports. Not on the critical path. (Was: `workflows/dashboard-sync.json` + Airtable mirror.)
...
- [X] T075 [US3] **API/webhook intake** `workflows/intake-webhook.sdk.ts` (`stTulzWEWMCS9qPS`) — see T053 (moved into Phase 8 delivery). Authenticated intake + create_campaign hardening.
- [X] T076 [Phone V2] **Phone Probe** — TWO pieces, both deployed:
  - **On-demand** `workflows/phone-probe.sdk.ts` (`8JLoJMdcFY8ylhzI`): POST {to} → Twilio AMD → classify (human/machine-voicemail/fax/no-answer). Manual/testing. Shares the `/leadgen-phone-twiml` endpoint.
  - **Fleet service** `workflows/phone-probe-service.sdk.ts` (`BP3pMFyvJ0n0bPLX`): warm-gated `phone_probe` service — migration `140_phone_probe_service.sql` (service CHECK +`phone_probe`), `commit_discovery_results` creates it, `complete_scorer_work_item` opens it on warm/hot, `complete_analysis_work_item` allowlist +`phone_probe`, reuses `phone_evidence→assessment`. Worker claims → Twilio call (n8n `Twilio account` credential, no inlined secret) → Wait 45s → poll → emits `phone_unanswered` (voice_ai, `when:true → +15` scoring rule) + `phone_probe_answered_by` → complete → re-score. Ships behind `service_config.phone_probe.enabled` (DISABLED — real outbound calls; flip true to activate). Twilio = Full account.
- [~] T077 [US2] **Contacts BLOCKED on a paid source** — reliable decision-maker contacts (name + valid email) are not obtainable free for SMBs (verified: Apollo free = org-only; Apify decision-maker actors are rental-gated / off-target; homepage email scrape yielded 0/3 usable). Business phone (Places) is captured on every lead and is the practical contact channel; strict **Hot** (verified personal contact) requires a paid source (Apollo paid or a rented Apify actor). Enricher is source-agnostic and will light up when a paid source is provided. See T044–T048.
...
- [X] T078 [Cost] Freshness cache in commit_discovery_results — for a REDISCOVERED business, reuse evidence observed within 14 days from any other campaign instead of re-running the paid analyzers. Helper leadgen._reuse_fresh_evidence copies latest-per-feature website/reviews/social/phone_probe evidence into the new campaign under a cache-reuse service_run + marks the analyzer work item done; phone (free) runs off the reused reviews. Only enabled/claimable services; prior campaign frozen; real observed_at preserved. Verified live: rediscovered med spas -> website/reviews/social served from cache (57 rows reused, 0 paid re-calls).
