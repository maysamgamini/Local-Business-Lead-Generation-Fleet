...
**Primary Dependencies**: n8n + Redis (queue mode); PostgreSQL; data providers — Google Places API, SerpAPI, Apify actors (Google reviews `compass~google-maps-reviews-scraper`, Yelp `tri_angle~yelp-review-scraper`, and — for the `social` service, 2026-07-18 — `apify~instagram-scraper` + `apify~facebook-pages-scraper` + `clockworks~tiktok-scraper`), Apollo (contacts), Hunter (email verify), PageSpeed Insights API; LLM providers — Anthropic Claude (judgment: site assessment, hot-lead critic), OpenAI + Google Gemini (bulk extraction/classification incl. `gemini-flash-latest` vision + report pitch; critics always cross-family from generator)

**Target market & signal design (2026-07-18)**: small local SMBs. This drives the signal stack toward what's observable at that scale — free homepage parsing (social links, tracking/ad pixels, chat + booking widgets) and Apify social-activity scraping (followers, last-post recency) — rather than website-traffic estimates. **Similarweb was evaluated and dropped**: its API needs ~5,000 monthly visits before it returns data, so it is empty for most SMB sites (see research). SerpApi + Apify cover the useful, obtainable signals.
