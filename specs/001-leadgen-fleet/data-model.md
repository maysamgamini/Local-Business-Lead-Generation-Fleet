...
| `revision_impact_rules` | Which cause re-queues which service | `cause_type`, `affected_service`, `enabled` | Self-requeue forbidden by default. Includes `social_evidence → assessment` (2026-07-18) so social-activity evidence re-scores the lead on a second pass |
...
**Initial states at discovery commit**: website+reviews → `pending`; phone → `blocked` (deps: website & reviews terminal); enrichment → `blocked` (resolves via scorer events + approval); **social → `blocked`** (warm-gated: the Scorer opens it `blocked → pending` when the lead is warm/hot; skipped otherwise — added 2026-07-18); assets → `blocked` (chain rules); missing prerequisite or a disabled `service_config` service → `skipped_prerequisite` + evidence fact.

**`work_items.service` enum** (migrations `030` + `130`): `discovery | website | reviews | phone | enrichment | assessment | assets | social` (`social` added 2026-07-18, `db/migrations/130_social_service.sql`, both namespaces).
...
| Entity | Purpose | Key fields | Notes |
|---|---|---|---|
| `evidence_items` | Immutable typed finding | `feature_key`, `value_jsonb` + `value_type` + `unit` + `confidence`, `calculation_version`, source fields, `content_hash`, `excerpt` (human explanation only), `service_run_id` FK, `UNIQUE(campaign_id, service, idempotency_key)` | Key = hash(campaign, service, feature_key, provider, source_record, observed_at, calc_version, content_hash). **Append-only: corrupt/superseded facts are repaired by appending a corrective observation (via `_insert_evidence`, latest-wins), never by deleting** (e.g. the 2026-07-18 `review_volume` clobber repair). **New feature_keys (2026-07-18):** `social_links`/object, `social_platform_count`/integer, `marketing_pixels`/object, `pixel_count`/integer, `chat_widget_present`/boolean, `booking_widget_present`/boolean, `web_features`/object (Website Auditor); `social_followers`/object, `social_last_post_days`/object, `social_inactive_90d`/boolean (Social Activity). `domain_hard_to_recall` is a Scorer-internal derived score component (no evidence row) |
...
**Classification rule** (campaign-relative): Hot `opportunity ≥75 AND contactability ≥60 AND confidence ≥60`; **Warm `opp ≥45`** (lowered 60→45 on 2026-07-18); Cold `≥40`; else disqualified. Thresholds are documented in the pinned scoring config set but are currently **enforced in the Scorer's Code node** (`r0K3xkLN2XtUceTF`), not read from config — see [contracts/scoring-defaults.md](./contracts/scoring-defaults.md) implementation note.
