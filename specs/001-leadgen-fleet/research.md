...
- **Decision (updated — redesign-focused, Gemini vision)**: The web/SEO product is *selling redesigns*, so the Website Auditor's job is to score **outdated / visually weak** sites high and capture material for a convincing report + the redesign. Signals: PSI ×3 (perf/seo/accessibility) + HTML freshness (Last-Modified header + footer copyright year → `staleness_years`) + viewport (`mobile_friendly`), then a **screenshot → Gemini Flash vision** pass producing `design_age`, `visual_appeal`, a quotable `redesign_rationale`, and brand colors. Screenshot via thum.io (free; swappable for self-hosted Browserless later). Vision model = **Gemini Flash** (cheapest production multimodal, image input included, ~fraction-of-a-cent/lead, already credentialed); Claude reserved for the cross-family hot-lead critic. GA4/Search Console deliberately excluded — they need the prospect's own authenticated access, impossible for cold leads (revisit only for onboarded clients). Superseded the original "PSI + text fetch_page crawl" plan: vision-on-a-screenshot judges UI/UX far better than a text crawl for the redesign pitch.
- **Original note**: Website — PageSpeed Insights API (Lighthouse lab) + tech fingerprints deterministically, then caged Claude agent (fetch_page, cap 6 calls). Reviews — Apify newest-200 processed oldest→newest, deterministic stats (trajectory, owner response rate), cheap-model theme extraction, quote verification gate. Phone — v1 passive from sibling evidence with `derived_from` lineage; v2 probe-caller drops in behind the same contract (compliance review first). Ads/creative — presence checks + rights-labeled asset references only.
